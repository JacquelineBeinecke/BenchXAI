"""
    ...
    and inside a folder for each class containing signal files.
...
class SignalDataset(Dataset):
    ...
    def __init__(self, root_path, transform=None):  # constructor
        ...
        for dic in sorted(os.listdir(root_path)):
            ...
            lab = torch.full([len(files)], int(dic))
    ...
    def __getitem__(self, idx):
        ...
        assert self.images[idx].split('.')[-1] == 'npy', "Signal files not in .npy format. Please store signal files " \
                                                         "as .npy files before loading."
        ...
        return image.to(torch.float32), self.label[idx], self.file_names[idx]  # the loaded filenames batch will be of type tuple
